...
Table~\ref{tab:scaling_amd} and Figure~\ref{fig:scaling_amd} show GPU and CPU frame time for all 11 molecules at 1080p on the AMD machine.
...
\begin{figure}[H]
    ...
    \caption{Per-pass breakdown across all 11 molecules (AMD, 1080p). SSAO and lighting are roughly constant because they depend on screen area; geometry and shadow grow with atom count.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{compare_scaling.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.68\textwidth]{load_amd.png}
...
\begin{enumerate}
    \item \textbf{Geometry submission is cheap.} Instanced rendering keeps the geometry pass below a few milliseconds even at 60k atoms.
    \item \textbf{SSAO is the main GPU cost.} It accounts for 40--50\% of the frame budget at 1080p. A half-resolution SSAO target with bilateral upsampling would have the largest impact on performance.
    \item \textbf{The pipeline scales predictably.} Fill-bound passes grow linearly with framebuffer area; the shadow pass is resolution-independent. No super-linear effects were observed.
\end{enumerate}

Comparing with the paper~\cite{sigg2006} in relative terms: they report the silhouette pass at roughly 8\% of the frame budget; ours is ~14--17\%. The difference is plausible given that our Sobel filter runs at full screen resolution, whereas theirs used a dedicated edge-detection pass.
